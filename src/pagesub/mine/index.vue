<template>
  <view class="page">
    ...
    <view class="list">
      <template v-if="list.length">
        <view class="item" v-for="(item, index) in list" :key="index">
          <view class="status-box">
            <view class="s-box">
              <image
                :src="`/static/images/mine/bg${
                  item.status === 1 ? 1 : item.status === 2 ? 2 : 3
                }.png`"
              ></image>
          ...
          <view class="btns-box">
            <view class="btn" v-if="item.status === 1" @click="refresh(item)"
              ><image src="/static/images/mine/ref.png" class="icon"></image
              >刷新</view
            >
            <view
              class="btn"
              ...
              ><image src="/static/images/mine/del.png" class="icon"></image
              >删除</view
            >
          </view>
        </view>
      ...
      <view class="btn2" @click="goTo('/pagesub/mine/clone')"
        >添加声音克隆</view
      >
...
const ok = () => {
  delVoiceClone({ id: currentId.value }).then((res) => {
    uni.showToast({ title: '删除成功!', icon: 'success', duration: 2000 })
    show.value = false
    const index = list.value.findIndex((v) => {
      return v.id === currentId.value
    })
