	...
	<view class="page">
		<view class="header-b">
			...
			<view class="info">
				...
				<view class="title"><u-icon name="arrow-left" color="#212121" size="36" class="icon" @click="back"></u-icon>用量管理
				</view>
				...
				<view class="menus_box">
					...
					<view class="i-d">
						<view class="c-b">
							...
							<view class="m-b">
								<view>总字{{ type === 1 ? '符' : '数' }}</view>
								<view class="data">{{ type === 1 ? user.info.f_byte : user.info.ali_byte }}字{{ type === 1 ? '符' : '数' }}
								</view>
							</view>
						</view>
						...
						<view class="c-b">
							...
							<view class="m-b">
								<view>已使用字{{ type === 1 ? '符' : '数' }}</view>
								<view class="data">{{ type === 1 ? user.info.f_byte_used : user.info.ali_byte_used }}字{{ type === 1 ?
									'符' : '数' }}</view>
							</view>
						</view>
					</view>
			...
			<view class="list-c">
				...
				<template v-if="data.length">
					<view class="item" v-for="(item, index) in data" :key="index">
						...
						<view class="r-c" :style="{ color: item.type === 1 ? '#F56C6C' : '#67C23A' }" v-if="type === 1">{{ item.type
							=== 1 ?
							'-' : '+' }}{{ item.byte_count }}字{{ type === 1 ? '符' : '数' }}</view>
						<view class="r-c" :style="{ color: item.type === 2 ? '#F56C6C' : '#67C23A' }" v-else>{{ item.type === 2 ?
							'-' :
							'+' }}{{ Math.abs(Number(item.amount_consumed)) }}字{{ type === 1 ? '符' : '数' }}</view>
					</view>
