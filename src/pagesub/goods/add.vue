...
<script setup>
	...
	import { goodsCategoryStore, goodsCategoryUpdate } from '@/api'
	...
	const add = async()=>{
		if(!form.value.name) return uni.showToast({title: '项目名称不能为空!',icon: 'none',duration: 2000});
		const response = form.value.id? await goodsCategoryUpdate(form.value) : await goodsCategoryStore(form.value)
		if(response){
