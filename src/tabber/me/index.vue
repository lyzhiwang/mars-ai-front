<template>
	<view class="page">
		<image src="/static/images/me/bg.png" class="topBg"></image>
		<view class="user_box">
			...
			<view class="info-box" v-if="user.isLogin">
				...
				<view class="user_b user_b1">
					<view>
						...
						<view class="time">
							有效期：{{ dayjs(user.info.start_time).format('YYYY-MM-DD') }}~{{ dayjs(user.info.end_time).format('YYYY-MM-DD') }}
						</view>
					</view>
			...
			<view class="info-box" v-else @click="goToLogin">
				<u-avatar class="avatar" :text="user.info.name.slice(0, 1)" fontSize="44rpx" size="120rpx"
		...
		<u-modal :show="showPwdPop" title="修改密码" showCancelButton @confirm="changePwdConfirm" @cancel="showPwdPop = false">
			<u--input placeholder="请输入您的新密码" border="surround" v-model="password" type="password"></u--input>
		</u-modal>
	</view>
</template>
